...
app.post('/api/auth/register', async (req: Request, res: Response) => {
  try {
    const { name, email, password, company_type, no_of_users, package_name } = req.body;
    let package_price = 49.99, plan_type = 'monthly';
    if (package_name === 'Pro')        { package_price = 99.99;  plan_type = 'annual'; }
    if (package_name === 'Enterprise') { package_price = 249.99; plan_type = 'annual'; }

    const password_hash = await bcrypt.hash(password, 10);
    const result = await pool.query(
      `INSERT INTO users (name,email,password_hash,role,company_type,package_name,package_price,no_of_users,plan_type,account_status,package_status,purchase_date)
       VALUES($1,$2,$3,'customer',$4,$5,$6,$7,$8,'active','active',CURRENT_DATE)
       RETURNING id,name,email,role,created_at`,
      [name, email, password_hash, company_type, package_name || 'Basic', package_price, no_of_users || 1, plan_type]
    );
...
app.post('/api/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, token, user: { id: user.id, name: user.name, email: user.email, role: user.role, account_status: user.account_status, package_name: user.package_name, package_status: user.package_status, purchase_date: user.purchase_date } });
...
app.get('/api/admin/stats', requireAdmin, async (_req, res: Response) => {
  try {
    const [total, active, suspended, revenue, pkgs] = await Promise.all([
      ...
      pool.query(`SELECT COALESCE(SUM(package_price),0) AS total FROM users WHERE role!='admin' AND account_status='active'`),
      pool.query(`SELECT package_name, COUNT(*) as count FROM users WHERE role!='admin' GROUP BY package_name ORDER BY count DESC`),
    ]);
...
app.get('/api/admin/users', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (status) { conditions.push(`account_status=$${idx}`); params.push(status); idx++; }
    if (pkg)    { conditions.push(`package_name=$${idx}`);   params.push(pkg);    idx++; }

    const where = `WHERE ${conditions.join(' AND ')}`;

    const [users, countResult] = await Promise.all([
      pool.query(
        `SELECT id,name,email,company_type,package_name,package_price,package_status,no_of_users,plan_type,account_status,purchase_date,created_at
...
app.get('/api/admin/users/:id', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [userResult, subStats, pkgResult] = await Promise.all([
      ...
      pool.query(
        `SELECT p.max_users,p.features,p.description FROM packages p
         JOIN users u ON u.package_name=p.name WHERE u.id=$1 LIMIT 1`,
        [userId]
      ),
    ]);
...
app.put('/api/admin/users/:id/status', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const r = await pool.query(
      `UPDATE users SET account_status=$1,status_reason=$2,status_changed_at=NOW(),status_changed_by=$3
       WHERE id=$4 RETURNING id,name,email,account_status`,
      [status, reason || null, req.adminUser!.id, req.params.id]
    );
  ...
  try {
    ...
    const mainUser = await pool.query(
      `SELECT u.name AS main_name, p.max_users,
       ...
       FROM users u LEFT JOIN packages p ON p.name=u.package_name WHERE u.id=$1`,
      [mainUserId]
    );
...
app.get('/api/admin/packages', requireAdmin, async (_req, res: Response) => {
  try {
    const r = await pool.query(
      `SELECT p.name AS package_name, p.plan_type, p.price AS package_price,
       ...
       FROM packages p
       LEFT JOIN users u ON u.package_name=p.name AND u.role!='admin'
       WHERE p.is_active=TRUE
       GROUP BY p.id,p.name,p.plan_type,p.price,p.description,p.features,p.max_users
       ORDER BY p.plan_type DESC, p.price ASC`
    );
...
app.post('/api/admin/packages', requireAdmin, async (req, res: Response) => {
  try {
    const { name, price, plan_type, description, max_users, features, is_active } = req.body;
    if (!name?.trim()) return res.status(400).json({ success: false, error: 'Package name is required.' });
    if (!price || isNaN(+price)) return res.status(400).json({ success: false, error: 'Valid price is required.' });
    if (!['monthly','yearly'].includes(plan_type)) return res.status(400).json({ success: false, error: 'Plan type must be monthly or yearly.' });

    const r = await pool.query(
      `INSERT INTO packages(name,price,plan_type,description,max_users,features,is_active)
       VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [name.trim(), +price, plan_type, description?.trim() || null, max_users ? +max_users : null, JSON.stringify(Array.isArray(features) ? features : []), is_active !== false]
    );
...
/** Check for subscriptions entering 7-day window and create notifications if not already created today */
async function checkExpiryNotifications() {
  try {
    const expiring = await pool.query(
      `SELECT u.id, u.name, u.email, u.package_name, u.plan_type, u.purchase_date,
              COALESCE(u.subscription_end_date,
                CASE WHEN u.plan_type='monthly' THEN u.purchase_date+INTERVAL '1 month'
                     WHEN u.plan_type='yearly'  THEN u.purchase_date+INTERVAL '1 year'
       ...
       WHERE u.role!='admin'
         ...
         AND (COALESCE(u.subscription_end_date,
               CASE WHEN u.plan_type='monthly' THEN u.purchase_date+INTERVAL '1 month'
                    WHEN u.plan_type='yearly'  THEN u.purchase_date+INTERVAL '1 year'
                    ELSE NULL END)::date - CURRENT_DATE) BETWEEN 0 AND 7`
    );
...
app.get('/api/admin/notifications', requireAdmin, async (req, res: Response) => {
  try {
    ...
    const [rows, countResult] = await Promise.all([
      pool.query(
        `SELECT n.*, u.name AS user_name, u.email AS user_email, u.package_name AS user_package
...
app.get('/api/admin/notifications/expiring', requireAdmin, async (_req, res: Response) => {
  try {
    const r = await pool.query(
      `SELECT u.id, u.name, u.email, u.company_type, u.package_name, u.plan_type,
              u.package_price, u.purchase_date,
              COALESCE(u.subscription_end_date,
                CASE WHEN u.plan_type='monthly' THEN u.purchase_date+INTERVAL '1 month'
                     WHEN u.plan_type='yearly'  THEN u.purchase_date+INTERVAL '1 year'
                     ELSE NULL END)::date AS end_date,
...
// PATCH mark all read (optionally by type)
