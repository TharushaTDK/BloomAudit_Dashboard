...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchCounts = useCallback(async () => {
    if (!token) return;
    try {
      const res = await api.get('/api/admin/notifications/counts', { headers });
  ...
  const fetchNotifications = useCallback(async () => {
    if (!token) return;
    try {
      const res = await api.get('/api/admin/notifications?limit=50', { headers });
  ...
  const markRead = useCallback(async (id: number) => {
    try {
      await api.patch(`/api/admin/notifications/${id}/read`, {}, { headers });
  ...
  const markAllRead = useCallback(async (type?: string) => {
    try {
      await api.patch('/api/admin/notifications/read-all', type ? { type } : {}, { headers });
