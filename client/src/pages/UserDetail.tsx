...
const UserDetail: React.FC = () => {
  ...
  const fetchUser = useCallback(async () => {
    try {
      const res = await api.get(`/api/admin/users/${id}`, { headers });
  ...
  const fetchSubUsers = useCallback(async () => {
    try {
      const res = await api.get(`/api/admin/users/${id}/sub-users`, { headers });
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await api.put(`/api/admin/users/${id}/status`, { status: newStatus, reason: statusReason }, { headers });
  ...
  const handleAddSubUser = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/api/admin/users/${id}/sub-users`, subForm, { headers });
  ...
  const toggleSubUser = async (subId: number) => {
    try {
      await api.patch(`/api/admin/sub-users/${subId}/toggle`, {}, { headers });
      fetchSubUsers();
      fetchUser();
  ...
  const deleteSubUser = async (subId: number) => {
    ...
    try {
      await api.delete(`/api/admin/sub-users/${subId}`, { headers });
      await Promise.all([fetchUser(), fetchSubUsers()]);
  ...
  const maxUsers = pkg?.max_users ?? user.no_of_users ?? null;
  ...
  // Subscription bar: percentage of time consumed (capped 0–100)
    ...
    <AdminLayout title="User Detail" subtitle="Full system profile and subscription overview">
      ...
      <div className="bg-slate-800/50 border border-slate-700/60 rounded-2xl p-6 mb-5">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {user.company_type && (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-700/50 text-slate-300 border border-slate-600/40">
                  <Building2 size={11} /> {user.company_type}
                </span>
              )}
              {user.package_name && (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-500/15 text-blue-400 border border-blue-500/30">
                  <Package size={11} /> {user.package_name}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-5 border-t border-slate-700/40">
          {[
            ...
            { label: 'Package',       value: user.package_name || '—',  icon: <Package size={13} /> },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
        ...
        <Section icon={<TimerReset size={16} />} title="Subscription" accent={health.color}>
          ...
          <Row label="Package Status">
            <span className={user.package_status === 'active' ? 'text-emerald-400' : 'text-slate-400'}>
              {user.package_status || '—'}
            </span>
          </Row>
        ...
        <Section icon={<Package size={16} />} title="Package Details" accent="text-indigo-400">
          <Row label="Package">{user.package_name || '—'}</Row>
          <Row label="Price">
            {user.package_price != null
              ? `$${Number(user.package_price).toFixed(2)} / ${user.plan_type === 'monthly' ? 'mo' : 'yr'}`
              : '—'}
          </Row>
          <Row label="Billing Cycle"><span className="capitalize">{user.plan_type || '—'}</span></Row>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
        <Section icon={<Mail size={16} />} title="Account Information">
          ...
          <Row label="Company Type">{user.company_type || '—'}</Row>
          <Row label="Source">{user.source || '—'}</Row>
          <Row label="Registered">{fmt(user.created_at)}</Row>
          {user.status_changed_at && <Row label="Status Changed">{fmt(user.status_changed_at)}</Row>}
          {user.status_reason && <Row label="Status Reason">{user.status_reason}</Row>}
        </Section>

        <Section icon={<DollarSign size={16} />} title="Billing Summary" accent="text-amber-400">
          <Row label="Package">{user.package_name || '—'}</Row>
          <Row label="Price Per Cycle">
            {user.package_price != null ? `$${Number(user.package_price).toFixed(2)}` : '—'}
          </Row>
          <Row label="Billing Cycle"><span className="capitalize">{user.plan_type || '—'}</span></Row>
